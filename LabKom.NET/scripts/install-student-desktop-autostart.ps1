# Daftarkan LabKom.Student.Desktop untuk auto-start saat user logon.
# Pakai Task Scheduler agar berjalan di Session 1 (interactive user session)
# dengan privilege user biasa.

param(
    [string]$TaskName = 'LabKomStudentDesktop',
    [string]$ExePath
)
...
if (-not $ExePath) {
    ...
    $candidate = Join-Path $root 'src\LabKom.Student.Desktop\bin\Release\net8.0-windows\LabKom.Student.Desktop.exe'
    if (-not (Test-Path $candidate)) {
        $candidate = Join-Path $root 'src\LabKom.Student.Desktop\bin\Debug\net8.0-windows\LabKom.Student.Desktop.exe'
    }
    if (-not (Test-Path $candidate)) {
        Write-Host "[!] LabKom.Student.Desktop.exe tidak ditemukan. Build dulu, atau berikan -ExePath." -ForegroundColor Red
        exit 1
    }
    $ExePath = $candidate
}
